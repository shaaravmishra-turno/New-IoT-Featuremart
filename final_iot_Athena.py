...
# Athena table for Mahindra (schema: camelCase columns, see convert_athena_mahindra_to_timescale_format)
ATHENA_MAHINDRA_TABLE = os.getenv("ATHENA_MAHINDRA_TABLE")
...
ATHENA_MAHINDRA_COLUMNS = (
    "vin",
    "lastconnected",   # -> last_connected -> EVENT_AT
    "soc",
    "batterytemp",     # -> battery_temp -> BATTERY_TEMPERATURE
    "kwh",             # -> POWER_DRAWN_KWH
    "odometer",
    "distancetoempty", # -> DISTANCE_TO_EMPTY
    "latitude",
    "longitude",
    "licenseplate",
    "vehiclemodel",
...
def _athena_configured():
    return _ATHENA_AVAILABLE and os.getenv("ATHENA_S3_STAGING")


def convert_athena_mahindra_to_timescale_format(df_athena):
    """
    Convert DataFrame from Athena mahindra_vehicles (camelCase columns, Athena types)
    to the same shape/names as Timescale mahindra_vehicle_data (snake_case, types expected by standardize).
    """
    if df_athena is None or df_athena.empty:
        return df_athena
    df = df_athena.copy()
    # Rename: Athena (camelCase) -> Timescale (snake_case)
    renames = {
    ...
    # Convert bigint (Unix ms) -> datetime for event_at and last_connected
    for col in ("event_at", "last_connected"):
        if col in df.columns:
            ser = pd.to_numeric(df[col], errors="coerce")
            df[col] = pd.to_datetime(ser, unit="ms", utc=True, errors="coerce")

    # Convert string/numeric columns to float where Timescale has float8
    numeric_cols = ["battery_temp", "soc", "kwh", "distance_to_empty", "odometer"]
    for col in numeric_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").astype(np.float64)

    # Ensure latitude, longitude are float (Athena double -> float)
    for col in ("latitude", "longitude"):
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").astype(np.float64)

    # Athena has no vehicle_speed, key_status, vehicle_mode, vehicle_status, latitude_direction,
    # longitude_direction, gps_validity_flag; standardize() will leave those as NaN
    return df


def _athena_partition_filter_for_last_n_days(days):
    """Build (year, month, day) IN (VALUES ...) for last `days` days for Athena partition pruning."""
    end = datetime.utcnow().date()
    start = end - timedelta(days=int(days))
    tuples = []
    d = start
    while d <= end:
        tuples.append((str(d.year), f"{d.month:02d}", f"{d.day:02d}"))
        d += timedelta(days=1)
    if not tuples:
        return ""
    # Athena/Presto: (year, month, day) IN (VALUES ('2026','02','18'), ...)
    values_sql = ", ".join(f"('{y}','{m}','{d}')" for y, m, d in tuples)
...
def _fetch_mahindra_athena_one_chunk(vins_chunk, days, chunk_id=None):
    ...
    partition_filter = _athena_partition_filter_for_last_n_days(days)
    sql = f"""
        SELECT {cols}
        FROM {from_clause}
        WHERE vin IN ({vin_list_sql})
        AND lastconnected >= {cutoff_ts}
        {partition_filter}
    """
    try:
        ...
        conn = athena_connect(**connect_kw)
...
def compute_features(df_std, days):
    ...
    for vin, subdf in df_std.groupby("VIN"):
        ...
        delta_t_hr = subdf["EVENT_AT"].diff().dt.total_seconds() / 3600
        # Avoid division by zero when consecutive rows have same timestamp
        delta_t_hr = delta_t_hr.replace(0, np.nan)
        subdf["ACCELERATION"] = subdf["VEHICLE_SPEED"].diff() / delta_t_hr
